...
#define DEFAULT_DITHER true        // dither even though it's only sometimes effective
#define DEFAULT_STORE_INTS false   // store data as scaled signed short integers

#define MAXSERIES 12           // maximum number of data series we plot
#define BLKDATASIZE 20000      // size in bytes of each series' data area in a block; must be divisible by 4
...
struct datablk_t {   // a block of up to NDATABLK data points for plotdata.nseries series
   ...
   int count;          // how many data are in the block
   union {             // the data of this block
      float fdata[1];      // either 4-byte floating point values
      int16_t idata[2];    // or 2-byte scaled signed integers
   } u; };
...
struct plotdata_t {   // info about our dataset
   ...
   bool store_ints;              // whether we should store scaled 2-byte ints instead of 4-byte floats
